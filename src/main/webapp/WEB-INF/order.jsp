				...
				<fieldset class="layui-elem-field layui-field-title"
					...
					<div class="layui-form-item">
						<label class="layui-form-label"></label>
						 <a class="layui-btn" href="${PATH}/order/excle">导出Excel</a>
						 <label class="layui-form-label"></label>
						 <a class="layui-btn" href="${PATH}/order/model">生成模板</a>
						 <label class="layui-form-label"></label>
...
<form class="layui-form layui-form-pane" action="${PATH}/order/updateActive" method="POST" id="updateRights">
  ...
  <div class="layui-form-item">
    ...
    <div class="layui-input-inline">
      <input type="text" id="prodCode" name="prodName" lay-verify="required" placeholder="请输入" autocomplete="off" class="layui-input">
    </div>
  </div>
    <div class="layui-form-item">
    <label class="layui-form-label">数量</label>
    <div class="layui-input-inline">
      <input type="text" id="count" name="count" lay-verify="required" placeholder="请输入" autocomplete="off" class="layui-input">
    </div>
  </div>
  <div class="layui-form-item">
...
<form class="layui-form layui-form-pane" action="${PATH}/order/insert" method="POST" id="registSubmit">
  ...
  <div class="layui-form-item">
    ...
    <label class="layui-form-label">商品编号</label>
    <div class="layui-input-inline">
      <input type="text" name="prodCode" lay-verify="required" placeholder="请输入" autocomplete="off" class="layui-input">
    </div>
  </div>
    <div class="layui-form-item">
    <label class="layui-form-label">数量</label>
...
layui.use('table',function(){
	var table = layui.table;
	  ...
	  table.render({
		    elem: '#demo'
		    ,height: 400
		    ,url: '${PATH}/order/selectModelPage' //数据接口
		    ,page: true //开启分页
		    ,where:{userCode: userCode,custCode: custCode}
		    ,cols: [[ //表头
		      {type:'checkbox', fixed: 'left'}
		      ,{field: 'userCode', title: '商品编号' }
		      ,{field: 'custCode', title: '商品名称' }
		      ,{field: 'prodCode', title: '产品编号' }
...
//监听工具行
  table.on('tool(test)', function(obj){
      ...
      if(obj.event === 'del'){
          ...
          layer.close(index);
          $.ajax({
              url:"${PATH}/order/deleteModel?userCode="+data.userCode,
              type:"GET",
    	...
    	layer.open({
	      ...
	      $.ajax({
               url:"${PATH}/pro/selectModel?userCode="+data.userCode,
               type:"GET",
               success:function(result){
